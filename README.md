...
Understanding the Node.js runtime and **Event Loop** is one of the most important foundations for becoming a good backend developer.

Before building APIs, authentication systems, payment integrations, or WebSocket servers, you need to understand:

> 🧠 **How Node.js actually executes your JavaScript code.**
...
`REST APIs` · `Web servers` · `Authentication systems` · `Real-time apps` · `CLI tools` · `Microservices` · `Streaming apps` · `Backend services`
...
`Parsing JS` · `Compiling JS` · `Executing JS` · `Memory management & garbage collection`

> ⚠️ V8 alone can't build a backend server — it doesn't know about `fs.readFile()` or `http.createServer()`. **Node.js provides those APIs around V8.**
...
## 7. ⏱️ Synchronous vs Asynchronous Execution
...
`Event loop` · `Async I/O` · `Networking` · `File-system operations` · `Timers` · `Thread pool`
...
> ⚠️ A timer sets a **minimum** delay before it's eligible to run — not a guaranteed exact time.

---

## 11. ⏲️ setTimeout vs setImmediate
...
## 14. ⚔️ CPU-Bound vs I/O-Bound Work
...
- [ ] **Task 1 — Execution Order:** Predict the output, then run it to verify:
  ```js
  console.log("A");
...
- [ ] **Task 3 — Blocking Experiment:** Write a CPU-intensive `while` loop and observe what happens to other timers or requests while it runs. Then research how **Worker Threads** solve this problem.
